...
namespace CleanArc_Kevin.Endpoints.API.Filters;
...
public partial class TrackActionPerformanceFilter : IActionFilter
{
    ...
    public void OnActionExecuting(ActionExecutingContext context)
    {
        ...
        var userDict = new Dictionary<string, string?>
        {
            ...
            { "OAuth2 Scopes", string.Join(",", context.HttpContext.User.Claims.Where(c => c.Type == "scope")?.Select(c => c.Value) ?? Array.Empty<string>()) }
        };
    ...
    public void OnActionExecuted(ActionExecutedContext context)
    {
        ...
        if (context.Exception == null)
            _logger.LogInformation(0, $"{context.HttpContext.Request.Path} {context.HttpContext.Request.Method} code took {_timer.ElapsedMilliseconds}.");
